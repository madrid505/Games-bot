...
async def handle_bank(update: Update, u_data, text, u_name, u_id):
    ...
    if cmd in ["راتب", "كنز", "بخشيش"]:
        ...
        msgs = {
            "راتب": f"💵 **مرسوم ملكي:** صرف راتب {base_amt:,} د. (ضريبة: {tax_amt:,} د). صافي: {net_amt:,} د.",
            "كنز": f"💎 **وجدتها!** عثرت على كنز بقيمة {base_amt:,} د. (ضريبة: {tax_amt:,} د). صافي: {net_amt:,} د.",
            "بخشيش": f"🎁 **بخشيش:** استلمت {base_amt:,} د. (ضريبة: {tax_amt:,} د). صافي: {net_amt:,} د."
        }
        await update.message.reply_text(msgs[cmd])
        return True

    # --- أوامر الاستثمار والمخاطرة (مع رقم) ---
    if cmd in ["استثمار", "حظ", "مضاربة"]:
        if len(parts) < 2:
        ...
        if random.random() > 0.5: # ربح
            ...
            await update.message.reply_text(f"📈 **ربحت!** القيمة: {win_amt:,} د. (ضريبة الدولة 10%: {tax_amt:,} د). دخل جيبك: {net_win:,} د.")
        else: # خسارة
            db.update({'balance': u_data['balance'] - amount}, User.id == u_id)
            await update.message.reply_text(f"📉 **خسرت!** فقدت {amount:,} دينار في {cmd}.")
        return True
    ...
    if cmd == "هدية" and update.message.reply_to_message:
        ...
        try:
            ...
            await update.message.reply_text("⚠️ اكتب مبلغاً صحيحاً تملكه.")
            return True
