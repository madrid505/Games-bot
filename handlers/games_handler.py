...
# 🖼️ ألبوم أساطير كرة القدم المحدث
SEASON_ALBUM = {
...
# 🖼️ دالات تحميل الصور (images.txt و contest_images.txt)
def load_image_quiz():
    quiz_data = []
...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "انا" and context.chat_data.get('r_on'):
        # مسموح تكرار "انا" ولكن التسجيل مرة واحدة في القائمة
    ...
    # 🏆 4. نظام توزيع البطاقات والجائزة الكبرى
    ...
    if img_ans and text == img_ans:
        ...
        await distribute_card(u_data) # النقاط مشتركة
        return

    # الألعاب النصية
    game_map = {"إسلاميات": "islamic", "ثقافة عامة": "general", "سيارات": "cars", "أندية": "clubs"}
    if text in game_map:
        key = game_map[text]; q = random.choice(QUESTIONS[key])
